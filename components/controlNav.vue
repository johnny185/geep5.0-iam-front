<template>
  <div class="nav">
    <ul class="navContent">
      ...
      <li class="navRight" v-if="$store.state.user.userInfo.token !== ''">
        <!-- 有用户时 -->
        <el-dropdown size="medium" v-if="$store.state.user.userInfo.nickName !== ''">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item @click.native="toPersonalCentre">个人信息({{$store.state.user.userInfo.registerType === 1 ? '认证中' :$store.state.user.userInfo.registerType === 2 ? '已认证' : '未认证'}})</el-dropdown-item>
        ...
        <!-- <div class="navItem borderLeft hoverStyle fontWhite" @click="toMessages">消息</div> -->
        <!-- <nuxt-link to="/?menu=2"> -->
          <!-- <span class="navItem borderLeft hoverStyle fontWhite" @click="toMenu('2')">消息</span> -->
          <!-- <messages
